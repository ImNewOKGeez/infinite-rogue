...
- `proximity_mine`: keeps the base proximity-mine trigger at `T1`, adds `6` smaller follow-up mines at `T2` that eject far outward at random angles and distances, increases only the main mine blast radius at `T3`, increases the scattered child-mine count to `10` at `T4`, and increases the main mine blast radius again at `T5`.
